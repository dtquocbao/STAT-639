...
There are census tracts with unusually high crime rates relative to the majority of observations. Tax rates and pupil-teacher ratios also show substantial variation, although the extreme skew is especially noticeable for crime.

We can inspect the largest values directly:
  
```{r ex10-largest}
head(Boston[order(Boston$crim, decreasing = TRUE), ], 10)
head(Boston[order(Boston$tax, decreasing = TRUE), ], 10)
head(Boston[order(Boston$ptratio, decreasing = TRUE), ], 10)
```

## (e) Census tracts bounding the Charles River

The variable `chas` equals 1 for census tracts that bound the Charles River.

```{r ex10-charles}
sum(Boston$chas == 1)
```

There are **`r sum(Boston$chas == 1)` census tracts** that bound the Charles River.
...
lowest_rows <- which(Boston$medv == min_medv)
lowest_rows

Boston[lowest_rows, ]
```

The minimum value of `medv` is **`r min_medv`**. Several tracts attain this minimum. To compare those observations with the overall ranges:
  
```{r ex10-lowest-comparison}
...
The lowest-value tracts tend to have characteristics associated with disadvantaged housing markets, including relatively high crime or lower-status population percentages and, in some cases, low average room counts compared with the rest of the data. The exact values are displayed above so they can be compared directly with each variable's overall range.

## (h) Census tracts averaging more than seven or eight rooms
...
There are **`r sum(Boston$rm > 7)`** census tracts with more than seven rooms per dwelling and **`r sum(Boston$rm > 8)`** with more than eight rooms per dwelling.

Inspect the tracts with more than eight rooms:

```{r ex10-rooms-eight}
rooms_gt8 <- Boston[Boston$rm > 8, ]
rooms_gt8

summary(rooms_gt8)
```

These tracts generally have large homes and relatively high median home values. However, they are not identical on the other predictors, so a large number of rooms by itself does not completely describe neighborhood conditions or housing value.
